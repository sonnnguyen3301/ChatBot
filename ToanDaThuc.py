...
class DaThuc:
	...
	domain = None
	is_vaild = False
	hasMultipleVariable = False
	hasNoVarialbe = False
	indexMultiVariable = -1
	indexNoVariable = -1
	...
	def Reset(self):
		...
		self.domain = None
		self.is_vaild = False
		self.hasMultipleVariable = False
		self.hasNoVarialbe = False
		self.indexMultiVariable = -1
		self.indexNoVariable = -1
		
	def CheckTerm(self, term, index):
		resultWork = []
		...
		# Kiểm tra số lượng biến của đa thức
		if len(poly_term.free_symbols) > 1:
			self.hasMultipleVariable = True
			self.indexMultiVariable = index
		elif len(poly_term.free_symbols) == 0:
			self.hasNoVarialbe = True
			self.indexNoVariable = index
	...
	def PassTerm(self, listTerm, domain=sp.RR):
		...
		for index, term in enumerate(listTerm):
			tempWork, poly_term = self.CheckTerm(term, index)
	...
	def NhanDaThuc(self):
		...
		if self.hasNoVarialbe:
			sentence = "Biểu thức " + "\(" + sp.latex(self.listTerm[self.indexNoVariable]) + "\)" + " không phải là đa thức mà chỉ là một con số. Mình chỉ thực hiện được phép nhân cho đa thức mà thôi."
			types = "Abort"
		...
		for i in range(1, len(self.listTerm)):
			...
			for arg in tempResult.args:
				if len(arg.free_symbols) == 0:
					sentence = "Cộng tất cả các phần không có chữ số lại, bạn được: " + "\(" + sp.latex(arg) + "\)" + ". <br>Kết quả hiện tại:"
				else:
					sentence = "Cộng tất cả các phần có chữ " + "\(" + sp.latex(sp.LM(arg)) + "\)" + " lại, bạn được: " + "\(" + sp.latex(arg) + "\)" + ". <br>Kết quả hiện tại:"
	...
	def ChiaDaThuc(self):
		...
		# Kiểm tra đa thức chỉ có 1 biến
		if self.hasMultipleVariable:
			sentence = "Biểu thức " + "\(" + sp.latex(self.listTerm[self.indexMultiVariable]) + "\)" + " có số lượng biến nhiều hơn một. Xin lỗi bạn nhưng mình chỉ chia được với số lượng biến là một mà thôi."
			types = "Abort"
			workList.append((None, types, None, sentence))
			return workList
		if self.hasNoVarialbe:
			sentence = "Biểu thức " + "\(" + sp.latex(self.listTerm[self.indexNoVariable]) + "\)" + " không phải là đa thức mà chỉ là một con số. Mình chỉ thực hiện được phép chia cho đa thức mà thôi."
			types = "Abort"
			workList.append((None, types, None, sentence))
			return workList

		# Kiểm tra mẫu có là bậc 1
		if sp.degree_list(self.listTerm[1])[0] != 1:
			sentence = "Xin lỗi bạn nhưng mình chỉ thực hiện được phép chia cho mẫu bậc 1 mà thôi."
			types = "Abort"
			workList.append((None, types, None, sentence))
			return workList
		...
		while len(arg.free_symbols) != 0:
			...
			if first:
				...
				sentence = "Chia bậc cao nhất ở 2 bên theo thứ tự trái qua phải: " + "\(" + sp.latex(left) + "\)" + " / " + "\(" + sp.latex(right) + "\)" + " = " + "\(" + sp.latex(divisor) + "\)" + ", ghi lại kết quả.\n<br>" \
							...
							"<br>Kết quả hiện tại: " + "\(" + sp.latex(result) + "\)" + "\n<br>" \
							"Biểu thức hiện tại: "
				types = "Explain"
				term = arg / rhs
				termLatex = "\(" + sp.latex(term) + "\)"
