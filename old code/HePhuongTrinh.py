...
class HPT:
	...
	def PassTerm(self, equationList, domain=sp.RR):
		...
		if not isLinear:
			sentence = "Phương trình: \n<br>" + sp.pretty(errorEq,use_unicode=True) + "\n<br>" + "không phải là phương trình tuyến tính, mình chỉ có thể giải được với các phương trình tuyến tính thôi."
			types = "Abort"
			resultWork.append((None, types, None, sentence))
			return resultWork, self.isVaild
	...
	def GiaiHPT(self):
		...
		# ------------ Kiểm tra xong ------------
		sentence = "Biểu thức bạn nhập vào sau khi mình rút gọn lại:<br>"
		term = ""
			...
			for index, eq in enumerate(self.equationList):
				...
				if first:
					sentence += "Đầu tiên bạn chọn phương trình nào có số biến ít nhất, nếu bằng nhau thì bạn chọn cái nào cũng được. " \
								"Mình sẽ chọn: \n<br>" + sp.pretty(eq, use_unicode=True) + "\n<br>"
				else:
				...
				if index != 0:
					...
					if first:
						sentence += "Nếu trong hệ phương trình đã có một phương trình có nghiệm rùi thì bạn chỉ cần thế nghiệm đó vào phương trình này.\n<br>" \
									"Cụ thể là thế: \n<br>" + prevEq + "vào\n<br>" + sp.pretty(oldEq, use_unicode=True) + "rút gọn lại bạn được\n<br>" + sp.pretty(eq, use_unicode=True) + "\n<br>"
					else:
						sentence += "Thế \n<br>" + prevEq + "\n<br>vào\n<br>" + sp.pretty(oldEq, use_unicode=True) + "\n<br>rút gọn lại được: \n<br>" + sp.pretty(eq, use_unicode=True) + "\n<br>"
				...
				sentence += "Hệ phương trình hiện tại:<br>"
				for eq in self.equationList:
					term += sp.pretty(eq, use_unicode=True) + "\n<br>"
